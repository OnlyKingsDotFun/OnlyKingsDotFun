...
import { Api } from "../../api";
import { TransactionV1 } from "./txV1";
...
export class TxBuilder {
  ...
  public async versionBuild<O = Record<string, any>>({
    ...
    if (txVersion === TxVersion.V1)
      return (await this.buildV1({ ...(extInfo || {}), lookupTableAddress })) as unknown as MakeTxData<TxVersion.V0, O>;
  ...
  /**
   * Transaction v1 (SIMD-0385): same account and instruction compilation as v0, 4,096-byte limit, and the
   * compute budget lives in the message config instead of ComputeBudget instructions. A v1 message with no
   * config is budgeted zero compute, so the limits below are always set; `computeBudgetConfig` values are
   * folded in when present. Requires web3.js >= 1.99 and an RPC that accepts `maxSupportedTransactionVersion: 1`.
   */
  public async buildV1<O = Record<string, any>>(
    ...
    const { transactionConfig, ...rest } = props || ({} as any);
    // Strip ComputeBudget instructions: in v1 they are message config, not instructions.
    const budgetIxs = this.allInstructions.filter((ix) => ix.programId.equals(ComputeBudgetProgram.programId));
    let computeUnitLimit = 1_400_000;
    let priorityFee: number | null = null;
    for (const ix of budgetIxs) {
      const tag = ix.data[0];
      if (tag === 2) computeUnitLimit = ix.data.readUInt32LE(1);
      if (tag === 3) {
        const microLamports = Number(ix.data.readBigUInt64LE(1));
        priorityFee = Math.ceil((microLamports * computeUnitLimit) / 1_000_000);
      }
    }
    const { forerunCreate, recentBlockhash: propRecentBlockhash, ...extInfo } = rest as any;
    ...
    // v1 has no address table lookups: compile the message with static keys only.
    const m0 = new TransactionMessage({
      payerKey: this.feePayer,
      recentBlockhash,
      instructions: this.allInstructions.filter((ix) => !ix.programId.equals(ComputeBudgetProgram.programId)),
    }).compileToV0Message([]) as MessageV0;
    if (this.owner?.signer && !this.signers.some((s) => s.publicKey.equals(this.owner!.publicKey)))
      this.signers.push(this.owner.signer);
    const txV1 = TransactionV1.fromV0(m0, {
      computeUnitLimit,
      heapSize: null,
    ...
    txV1.sign(this.signers);
    // Hand out a web3.js VersionedTransaction (version 1) so wallets and RPC helpers see a normal object;
    // its `serialize()` is re-pointed at the v1 encoder because web3.js 1.99 only reads v1.
    const transaction = txV1.toVersionedTransaction();
    (transaction as any).serialize = () => txV1.serialize();
    (transaction as any).sign = (signers: Signer[]) => {
      txV1.sign(signers);
      transaction.signatures = txV1.signatures;
    };
    (transaction as any).addSignature = (pk: PublicKey, sig: Uint8Array) => {
      txV1.addSignature(pk, sig);
      transaction.signatures = txV1.signatures;
    };
    return {
      builder: this,
      ...
      execute: async (params) => {
        ...
        if (this.owner?.isKeyPair) {
          ...
          if (sendAndConfirm) await confirmTransaction(this.connection, txId);
          return { txId, signedTx: transaction };
        }
        if (this.signAllTransactions) {
          ...
          return { txId: notSendToRpc ? "" : await this.connection.sendTransaction(txs[0], { skipPreflight }), signedTx: txs[0] };
        }
        throw new Error("please provide owner in keypair format or signAllTransactions function");
      },
      extInfo: (extInfo || {}) as O,
    } as MakeTxData<TxVersion.V0, O>;
  }
