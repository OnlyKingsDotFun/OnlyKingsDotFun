...
- fork demos against the live WSOL/USDC CLMM pool, on a Surfpool mainnet fork and on an Agave 4.3 `solana-test-validator` with the pool, config, vaults, observation, bitmap extension and 13 tick arrays cloned: against the live WSOL/USDC pool `3ucNos4NbumPLZNWztqGHNFFgkHeRMBQAVemeeomsUxv`: standard `swap_v2` charged 400 ppm, `rebalance_swap_v2` toward target charged 4 ppm; balanced-pool, same-direction and overshooting rebalances rejected with `NotRebalancing`.

## tests

`svm-tests/` (standalone crate, own lockfile) runs the compiled program under [LiteSVM] on real mainnet state: the live WSOL/USDC pool `3ucNo…`, its config, vaults, observation, bitmap extension and the tick arrays around the current tick. Three tests:

- admin gating (negative; the localnet admin key is not distributed, so rulesets are seeded for the permissionless paths)
- collections and members: every instruction, every rule branch (`Any`, `ImmutableMint`, pump.fun standard / mayhem / allow-mayhem / require-complete / wrong owner / wrong discriminator / wrong curve / no proof), rates
- `rebalance_swap_v2`: balanced pool rejected both ways, unchanged `swap_v2` fee (400 ppm recovered from the protocol fee delta), same direction rejected, toward target at /100 and /500, 1 ppm floor, overshoot rejected, wrong member PDA, missing member
